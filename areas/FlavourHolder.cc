...
FlavourHolder::FlavourHolder(int idhep): _flav_content(7), _idhep(idhep) {
  ...
  // default is no flavour content
  for (int i = 0; i <=6; i++) {_flav_content[i] = 0;}

  if (ndigits == 1) { // a lone quark
    if (digit[0] > 6 || digit[0] == 0) {
      cerr << "Failed to understand idhep = "<<_idhep<<endl; exit(-1);}
    ...
    for (int i=1; i < ndigits; i++) {
      if (digit[i] > 6) {cerr << "Failed to understand idhep = "
			       <<_idhep<<endl; exit(-1);}}
