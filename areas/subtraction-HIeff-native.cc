...
//     double a, b;
//     full_seq.parabolic_pt_per_unit_area(a,b,-1.,10000.);
//     cout << "a, b = " << a << " " << b << endl;
...
//     }
    ...
    for ( int i = 0; i < nr; i++ ) {
      ...
      // some printout
      //vector<fj::PseudoJet> tmp = sorted_by_pt(full_jets_iter);
      //for(int k = 0; k < 4; k++ ){
      //   cout << k << " " << " " << tmp[k].rap() << " " << tmp[k].phi() << " " << tmp[k].perp() << endl;
      //}
      ...
      typedef vector<fj::PseudoJet>::const_iterator jet_iter;
      for (jet_iter jet = full_jets_iter.begin(); jet != full_jets_iter.end(); jet++) {
        if (abs(jet->rap()) < active_area_spec.ghost_etamax() - 1.0) {
      ...
      inputs = full_corrected_jets_iter;

      // some printout
      //vector<fj::PseudoJet> tmp2 = sorted_by_pt(inputs);
      //for(int k = 0; k < 4; k++ ){
...
//     reorder_jets(hard_jets, inputs, max_rapphi_dist, nmatch);
// 
//     // --- print out some info
//     cout << "Matched "<<nmatch<<" jets"<< endl;
//     for (unsigned i = 0; i < nmatch; i++) {
//       cout << i << " " << hard_jets[i].perp() << " " << full_jets[i].perp() << " " << inputs[i].perp() << " " << hard_jets[i].squared_distance(inputs[i]) << endl ; 
//     }
//     cout << " +++++++ " << endl;
// 
    ...
    for (unsigned i = 0; i < hard_jets.size(); i++) {
      ...
      if (i < nmatch) {
        ...
        matched_areas[ipt].add_entry(full_seq_iter->area(full_corrected_jets[i])
                                                            /reference_area);
      } else {
        pt_lost_entries[ipt]++;
      }
    ...
    for (unsigned i = nmatch; i < full_corrected_jets.size(); i++) {
      ...
      all_areas.add_entry(full_seq_iter->area(full_corrected_jets[i])
                          /reference_area);
      ...
      fake_areas[ipt].add_entry(full_seq_iter->area(full_corrected_jets[i])
                                                            /reference_area);
    }
