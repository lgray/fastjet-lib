...
// try to find out something about the object
void print_info(const Polygon_2 & poly) {
  Traits_2::Curve_const_iterator it;
  for (it = poly.curves_begin(); it != poly.curves_end(); it++) {
    std::cout << "Found an edge: " << it->is_circular() << std::endl;
  }
}
...
// Construct a rectangle from points
...
int main ()
{
  ...
  //std::cout << "Area of circ1 is " << circ1.area() << std::endl;
