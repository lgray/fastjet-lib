...
%
They are visually obvious structures when one looks at an experimental
event display, and by measuring their energy and direction one can
approach the idea of the original ``parton'' that produced them.
%
...
%
Suitably defined jet algorithms can be applied to particles,
calorimeter towers, or even to the partonic events of perturbative QCD
calculations, with the feature that the jets resulting from the
various different kinds of input are not just physically close to the
...
field~\cite{Moretti:1998qx,Ellis:2007ib,Salam:2009jx,JetsHistory}.
%
...
straightforward, efficient access to the majority of widely used
sequential-recombination and cone-type jet algorithms, both for
hadron-hadron and $e^+e^-$ colliders.
...
algorithms, has played a key in the design of \fastjet's interface.
%
...
\end{equation}
%
$R$ in eq.~(\ref{eq:dij-illustr}) is a parameter of the algorithm that
determines its angular reach.
%
...
``pseudojet'', since it is neither a particle, nor yet a full jet.  In
\fastjet we will use this name to denote any generic object with
4-momentum.
%
...
perform the clustering, which quickly becomes prohibitive.
%
...
eqs.~(\ref{eq:dij-illustr},\ref{eq:diB-illustr}), and they were also
widely used at HERA and LEP.
%
...
with \fastjet. 
%
Together with the native algorithms, they ensure easy access to all
jet algorithms used at colliders in the past decade
...
etc.
%
\comment{NB: the above phrasing is intended to avoid risk of
  third-party cone authors saying ``that's my code you've got in
  there, I too should be an author of \fastjet''. I.e. we provide an
  uniform interface to the code, distribute it for convenience, and do
  a little curation. But it is not our code.}
...
%
However, even in events at scales $\gg M_{EW}$, there can
...
%
Version 3 of \fastjet aims to improve this situation, providing example
...
jet-by-jet subtraction of the contamination~\cite{cs}.
%
...
%
Additionally, extensive online ``doxygen'' documentation is available
online at \url{http://fastjet.fr}.
...
around Eq.(\comment{InIntro}).
%
For nearly all hadron-collider algorithms, the ``inclusive'' jets are
the ones usually just referred to as the ``jets''.
...
\begin{lstlisting}
  ...
  /// Note this would always return 0 if active_area_explicit_ghosts is used.
...
\begin{lstlisting}
  ...
  /// true if a jet is made exclusively of ghosts (and the AreaType is
  /// active_area_explicit_ghosts)
  /// It also applies for constituents of a jet, in which case it will
...
The first technique, ``local selector'', available for now only in
...
The second technique, ``rescaling'', was suggested by unpublished
studies for the case of $pp$ pileup events (with Pythia~8.145, tune 4C
for the pileup~\cite{Sjostrand:2007gs}). We have found that an
alternative method that is adequate for handling the rapidity
dependence is to parametrise the average shape of the rapidity
dependence from some number of pileup events and then carry out an
