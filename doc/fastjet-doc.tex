...
\end{lstlisting}
(for other shapes, derive a class from \ttt{BackgroundRescalingBase}
\comment{check the name stays the same}) and then one tells the
...
\ttt{BackgroundJetDensityBase::operator(PseudoJet \& jet)} function is
called and it returns the quantity that substitutes $p_t/A$.
%
