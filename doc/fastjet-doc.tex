...
Being a jet constituted by a finite number of particle, the notion
of its {\sl area} (i.e. the surface in the $\eta$-$\phi$ plane over 
which it extends) cannot be a rigorous one. It is however possible to 
make this concept better defined via a proper definition.
...
of the various runs. If \ttt{repeat} $ > 1$ a numerical
...
Virtual base class that sets the interface for extensions 
...
in the clustered event. These methods might migrate to the virtual base class in
the future.
\begin{verbatim}
...
being parabolic cin $\eta$) in simulated Pb-Pb collisions at the LHC.
