...
\title{FastJet 3.0beta2-devel user manual}
...
Currently the $pp$ Cambridge/Aachen jet algorithm is provided only in an
...
Each different kind of warning is written out a maximum number of times  (the current
default is 5) before its output is suppressed. The program is allowed to continue. It is
possible to obtain at the end of the run (or whenever one wishes to do so) a summary of
all the warnings, both explicit or supressed, by defining and using a
\ttt{LimitedWarning}  object as follows:
\begin{lstlisting}
...
\end{lstlisting}
to control whether an error message is printed (default = \ttt{true}) and whether a full
backtrace is also given (default = \ttt{false}).
...
\begin{lstlisting}
  ...
  #include <fstream>
  // ...
  ofstream myerr("warnings-and-errors.txt");
  LimitedWarning warnings;
  warnings.set_default_stream(&myerr);
  Error errors;
  errors.set_default_stream(&myerr);
  errors.set_print_backtrace(true);
  // ...
  cout << warnings.summary() << endl;
 
\end{lstlisting}
will send the output of both classes to the file \ttt{warnings-and-errors.txt} (as well
as provide the backtrace of errors).
Note that the output of \ttt{warnings.summary()} still goes to standard output, and will
only be present if the program has not aborted earlier due to an error.
