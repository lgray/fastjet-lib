...
% geometrically-based C++ implementations \cite{fastjet} for a number of
% sequential recombination jet algorithms, as well as plugins for many
% widely used cone algorithms.
% 
% the longitudinally
...
\ttt{Rcut}, with \ttt{Rcut} = \ttt{Rcut\_factor}$\times 2m/p_t$ 
\item $p_t^i, p_t^j >$~\ttt{zcut}$\times
p_t^{i+j}$
\end{enumerate}
If both these criteria fail, $i$ and $j$ are not recombined, the harder of $i$
...
follows, using a \ttt{JetDefinition} and two \ttt{double} parameters:
\begin{lstlisting}
  #include "fastjet/tools/Pruner.hh"
  // ...
  Pruner pruner(subjet_def,  zcut,  Rcut_factor);
  // ...
...
The jet definition \ttt{subjet\_def} used in hte \ttt{Pruner} to recluster the
constituents of a jet should normally have a radius large enough to ensure that
all its constituents are reclustered into a single jet. Should this not happen,
pruning is applied to the hardest of the jets found, and the others can be
retrieved using
\begin{lstlisting}
   vector<PseudoJet> extra_jets = pruned_jet.structure_of<Pruner>().extra_jets();
\end{lstlisting}

Finally, note that a second constructor for \ttt{Pruner} exists, that allows one
to construct the pruner using functions that compute dynamycally \ttt{zcut} and
