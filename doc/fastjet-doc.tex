%% TO DO before a 3.0:
%% - make sure references to 3.0(alpha) are all dealt with
%% - discuss UserInfo in section 3?
%% - section 7.5 background subtraction
%% - section 7.5 background estimation example
%%
%% TO DO for arXiv release?
%% - rewrite abstract
%% - move description of native jet algorithms to start of document?
%%
%% TO DOcument for 3.0alpha2
%% - join(...) [done]
%% - check UserInfo part up to date [done]
%% - check substructure of jets part up to date [done]
%% - tools
%%
%% TO DO:
%%
...
\title{FastJet 3.0alpha3-devel user manual}
...
\end{lstlisting}
A more powerful method involves a pointer to a derived class of
\ttt{PseudoJet::UserInfoBase}. It is discussed in
appendix~\ref{app:user-info}. 

A \verb:PseudoJet: can be reset with
\begin{lstlisting}
  /// reset the 4-momentum according to the supplied components,
  /// put the user and history indices back to their default values (-1
...
\end{lstlisting}
The 4-momenta are added, and in addition the \ttt{resonance} remembers
that it came from \ttt{jet1} and \ttt{jet2}. So, for example, a call
to \ttt{resonance.constituents()} will return the constituents of both
\ttt{jet1} and \ttt{jet2}.
%
It is possible to \ttt{join} 1, 2, 3 or 4 jets or a \ttt{vector} of
jets.

For a jet formed in this way, one can find out which pieces it has
been composed from with the function
\begin{lstlisting}
  vector<PseudoJet> pieces = resonance.pieces();
\end{lstlisting}
which, in the above example, would simply return a vector of size 2
containing \ttt{jet1} and \ttt{jet2}.
%
For jets not formed in this (or a related) way \ttt{pieces()} will
throw an error.



%......................................................................
\paragraph{Inquiries as to available structural information.}
...
always be clear kinds of structural information it has. 
%
Accordingly a number of inquiry functions are available:
\begin{lstlisting}
  bool has_structure();         // returns true if it has some kind of structural info
  bool has_constituents();      // returns true if it has constituents 
...
\end{lstlisting}
The dichotomy between cluster-sequence type structural information and
\ttt{pieces} helps provide a distinction between different origins for
structure --- either that arising diretly from a jet algorithm, or
that applied a posteriori. 
%
There is a balance between the usefulness of such a dichotomy and the
added complication it brings.
%
In this respect version 3.0alpha3 should be considered a snapshot of
ongoing thought about the question.


%......................................................................
\paragraph{Features not yet documented}

The following features still need documenting.
\begin{lstlisting}
  PseudoJetStructureBase;
  structure<StructureType>();
  has_structure_of<ToolName>();
  structure_of<ToolName>();
\end{lstlisting}
...
\ttt{()} having in C++  higher precedence than boolean operators.
...
Mostly for completeness, there is a \ttt{SelectorIdentity} which
selects everything. And, as a helper for the situations where ghost
areas are computed using explicit ghosts, \ttt{SelectorIsPureGhost}
selects only pure-ghost jets.
...
%======================================================================
\section{Jet areas, background evaluation and subtraction}
\label{sec:areas}
...
\begin{lstlisting}
   class fastjet::BackgroundEstimator
   class fastjet::Subtractor
\end{lstlisting}
These classes are described in detail below and an example program is
given in section~\ref{sec:ex-prog-sub}. \comment{Check it before release...?}
...
\begin{lstlisting}
  fastjet::GhostedAreaSpec(const Selector & selector, 
                           int    repeat        = 1,
                           double ghost_area    = 0.01, 
                           double grid_scatter  = 1.0, 
                           double kt_scatter    = 0.1,
...
% Jet areas can be of use in determining the level of a randomly
% distributed diffuse background that might be present together with the
% hard event of interest, whether from the underlying event or pileup.
% %
% After selecting a phase space region over which to analyse the jets,
% one can determine the average level $\rho$ (transverse momentum per
% unit area) of the background, and also subtract it from the hard jets.

One of the difficulties in characterising the background is that it is
...
%
This is insensitive to the presence of a handful of hard jets, and
...
algorithms for calculating the UE/pileup density $\rho$ in this
way. In particular, it is important to avoid algorithms with extreme
behaviours for the areas. 
%
Specifically, one should not use anti-$k_t$ (which has many jets with
near-zero area), or SISCone (which can have jets with near-zero area,
as well as monster jets with huge areas if the overlap parameter $f$
is too small).
%
The choice of jet radius can also have an impact on the determination,
especially for events that are relatively sparse.
%
We have found that a choice in the range $R=0.4-0.6$ is generally
adequate~\cite{cs,Cacciari:2009dp} with a preference for larger values
if the events are relatively sparse and smaller values if you expect
your events to have many hard jets.

% an appropriate value of $R$, typically of order 0.4--0.6,
% see~\cite{cs,Cacciari:2009dp} for more details), its value can then be
% used for UE/pileup subtraction in an analysis using any jet algorithm.


%In contrast the $k_t$ and
%Cambridge/Aachen algorithms are suitable.

% Once $\rho$ has been determined using $k_t$ or Cambridge/Aachen (and
% an appropriate value of $R$, typically of order 0.4--0.6,
% see~\cite{cs,Cacciari:2009dp} for more details), its value can then be
% used for UE/pileup subtraction in an analysis using any jet algorithm.
...
Two elements are needed for FastJet's background estimation: a cluster
sequence with area information and a \ttt{Selector} to specify the
subset of jets that will be used for the background estimation.
%
The class that provides background estimation facilities is
\ttt{BackgroundEstimator}. The simplest way to construct it is:
\begin{lstlisting}
   #include "fastjet/tools/BackgroundEstimator.hh"
   // ...

   // constructor for a BackgroundEstimator that will use the
   // inclusive jets from csa that pass the cuts specified by
   // the selector in order to estimate the background density
   BackgroundEstimator(const ClusterSequenceAreaBase & csa,
                       const Selector & selector);
\end{lstlisting}
If, for example, one wants to use all jets within $|y|<4.5$ to
estimate the background, one should choose \ttt{selector =
  SelectorAbsRapMax(4.5)}. The background density $\rho$ and its
fluctuations $\sigma$ can then be accessed through the functions:
\begin{lstlisting}
   // the median of ($p_t$/area) for jets that pass the selection cut, 
   // making use also of information on empty area in the event
   double rho()   const; 
...
the selected region as consisting of jets of zero $p_t$.
%
Thus (roughly speaking), if more that half of the area covered by the
selector is empty the median estimate for $\rho$ will be zero.
%
\comment{empty areas should be discussed more in comments to empty
  area calls below --- check it's OK later}

%......................................................................
\subsubsection{Positional dependence of background}
\label{sec:BGE-positional}

One drawback of a $\rho$ estimate based on all jets from (say)
$|y|<4.5$ is that the background density in $pp$ and heavy-ion
...
%
The first involves the use of a more local range for the determination
of $\rho$, with the help of a \ttt{Selector} that is able to take a
reference jet, e.g. \ttt{SelectorStrip(}$\Delta y$\ttt{)}, a strip of
half-width $\Delta y$ (which might be of order $1$) centred on
whichever jet is set as its reference.
%
With this kind of selector, when the user calls either of the functions
\begin{lstlisting}
...
selector on the specified jet. Then the inclusive jets in \ttt{csa}
...
that average shape.
%
One first creates an object that encodes the shape
\begin{lstlisting}
   // gives rescaling$(y) = 1.16 + 0\cdot y -0.023 \cdot y^2 + 0\cdot y^3 + 0.000041 \cdot y^4$
   BackgroundRescalingYPolynomial rescaling(1.16, 0, -0.023, 0, 0.000041);
\end{lstlisting}
(for other shapes, derive a class from
\ttt{FunctionOfPseudoJet<double>} --- see appendix
\ref{app:function-of-pj}) and then one tells the
\ttt{BackgroundEstimator} about the rescaling with the call
\begin{lstlisting}
   // tell the BackgroundEstimator about the rescaling
   set_rescaling_class(*rescaling);
\end{lstlisting}
Subsequent calls will then take the median of the distribution $p_t/A
/ \ttt{rescaling}(y)$ (rather than $p_t/A$) and any calls to
\ttt{rho(jet)} and \ttt{sigma(jet)} will including an additional
...
%%   virtual bool is_pure_ghost(const PseudoJet & jet) const ;
%% 
A number of enquiry functions are available to obtain information used
internally within the median $\rho$ and $\sigma$ determination.
\begin{lstlisting}
   // Returns the mean area of the jets used to actually compute the background properties
   // (including empty area and jets)
...
to \ttt{rho} or \ttt{sigma} (as long as the cluster sequence or
selector has not changed in the meantime).

To allow flexibility in the user's workflow, there are a number of
alternative constructors to \ttt{BackgroundEstimator}, together with
elements to (re)set the cluster sequence and/or jets, and also the
selector:
\begin{lstlisting}
  // the estimator will use the supplied jets instead of inclusive jets from a ClusterSequence
  BackgroundEstimator(const std::vector<PseudoJet> & jets, const Selector & rho_range);
  // information about the ClusterSequence or jets will be set later
  BackgroundEstimator(const Selector & rho_range);
  // information about the ClusterSequence/jets and Selector will be set later
  BackgroundEstimator();
...
\end{lstlisting}
Jets passed to any of the above calls must originate from a common
originate from a common \ttt{ClusterSequenceAreaBase} type class.

Finally, there are cases where the user may wish to determine medians
of quantities other than $p_t/A$. 
%
This may arise in estimating the hadron-mass contribution to pileup
contamination of jet masses, or in the determination of moments of
fragmentation functions.
%
For this purpose, the user may use the following call
\begin{lstlisting}
  void set_jet_density_class(const FunctionOfPseudoJet<double> * jet_density_class);
\end{lstlisting}
For each jet that passes the selection, \ttt{jet\_density\_class}'s
\ttt{FunctionOfPseudoJet<double>::operator(PseudoJet \& jet)} function
is called\footnote{As explained in Appendix \ref{app:function-of-pj}
  describing functions of \ttt{PseudoJet}, user-defined density
  classes derived from \ttt{FunctionOfPseudoJet<double>} should
  overload \ttt{FunctionOfPseudoJet<double>::apply()}.} and it returns
the quantity that substitutes $p_t/A$.
%
\comment{Do we discuss the \ttt{BackgroundJetPtMDensity} class as an
  example here?}
...
The \ttt{BackgroundEstimator} class is new to FastJet 3.0.
%
...
and that is now the default in \ttt{BackgroundEstimator}. 
%
...
handled the limit of a small number of jets. A call to
\ttt{set\_provide\_fj2\_sigma(true)} causes \ttt{BackgroundEstimator}
...
\comment{Fill this in with the new Subtractor}
...
%   // "error" (uncertainty), which is obtained from the 1-sigma
%   // half-width of the distribution of pt/A. To be precise it is
%   // defined such that a fraction (1-0.6827)/2 of the jets
%   // (including empty jets) have $p_t/A < \rho - \sigma \sqrt{\langle A \rangle}$
%   //
...
% %% 
% \begin{lstlisting}
%   // return the total area, within the range, that is free of jets
...
\comment{This subsecction still relates to the version 2.x subtraction
  interface in \fastjet.}
...
%======================================================================
\section{Jet transformers (substructure, etc...)}
\label{sec:transformers}

Performing post-clustering actions on jets has become very common: for
example, various techniques have been introduced to tag boosted
objects. We introduced (from \fastjet 3.x onwards) a common interface
to ease the usage of these tools and the creation of new ones.

Below, we first discuss generic considerations about these tools that
we call {\ttt Transformer}'s, then we go through the list of the ones
that have already been implemented. We then dig into the more
technical aspects related to how they are implemented and how to add
user-defined transformers.


\subsection{Generic considerations about \ttt{Transformer}'s}
\label{sec:transformers}

The common interface to all the transformers is the following one:
\begin{lstlisting}
...
This contains two concepts: first, the basic action of the transformer
through \ttt{apply} would return a transformed jet from the one passed
as an argument. Note that, in practice,
\begin{lstlisting}
  TOut operator()(const PseudoJet &pj) const;
  vector<TOut> operator()(const vector<PseudoJet> &pjs) const;
\end{lstlisting}
would equivalently return the action of the transformer on one or a
vector of \ttt{PseudoJet}, since \ttt{Transformer} is declared as a
function of \ttt{PseudoJet} returning a \ttt{PseudoJet} (see Appendix
\ref{app:function-of-pj}):
\begin{lstlisting}
  MyTransformer transformer;
  PseudoJet transformed_jet = transformer(jet);
\end{lstlisting}
Then, transformers are likely to bring new structural information with
the returned result. For a given transformer, say \ttt{MyTransformer},
the new information that is not directly accessible from
\ttt{PseudoJet} (like its \ttt{constituents}, \ttt{pieces} or
\ttt{area} when they are available), coulb be accessed through
\begin{lstlisting}
  jet.structure_of<MyTransformer>()
\end{lstlisting}
which would give direct access to the structural information produced
by \ttt{MyTransformer}. Using
\ttt{jet.has\_structure\_of<MyTransformer>()}, it is possible to check
if \ttt{jet} is compatible with the structure provided by
\ttt{MyTransformer}.


\subsection{Jet Filtering using \ttt{Filter}}
\label{sec:filtering}
...
Here, we decided to take the most generic approach by introducing
\begin{lstlisting}
...
\end{lstlisting}
which would obtain the subjets by reclustering the jet's constituents
with the jet definition \ttt{subjet\_def} and use \ttt{selector} to
decide which of these subjets have to be kept. If \ttt{rho} is
non-zero, each of the subjets will be subtracted (using the specified
value for the background density) prior to the selection of the kept
subjets.

Two alternative constructors are supported:
\begin{lstlisting}
...
the full subjet definition. For the second, one has to provide a
(pointer to) a class derived from \ttt{FunctionOfPseudoJet<double>}
that would dynamically compute the filtering radius as a function of
the jet being filtered (as it was originally proposed in \cite{BDRS}
where $R_{/rm filt}={\rm min}(0.3,R_{b\bar{b}})$ depends on the object
being filtered). In both cases, the Cambridge/Aachen algorithm is used.

\paragraph{Constraints.} The jet being filtered must have
constituents. Furthermore, if \ttt{rho} is non-zero, the input jet
must be the result of a \ttt{ClusterSequence} with area support and
explicit ghosts. If any of these requirements fail, an exception will
be thrown.

\paragraph{Structure of the result.} The \ttt{PseudoJet} resulting
from the filtering will have an associated structure of type
\ttt{FilteredJetStructure}, derived from
\ttt{CompositeJetStructure}. On top of the subjets kept by the filter,
available as its \ttt{pieces} (see Section \ref{sec:composite-jet}),
we store the rejected pieces and the original (input) jet. They are
accessible using
\begin{lstlisting}
  vector<PseudoJet> rejected = jet.structure_of<Filter>().rejected();
  PseudoJet original = jet.structure_of<Filter>().original();
\end{lstlisting}

\paragraph{Usage examples.} Filtering involving obtaining the subjets
with the Cambridge/Aachen algorithm with radius $R_{/rm filt}$ and
keeping the $n_{/rm filt}$ hardest subjets found can be achieved using
\begin{lstlisting}
  Filter filter(Rfilt, SelectorNHardest(nfilt));
\end{lstlisting}
Similarily, trimming, keeping the jets with a $p_t$ larger than a
fixed fraction of the input jet, is done using
\begin{lstlisting}
  Filter trimmer(subjet_def, SelectorPtFractionMin(pt_fraction_min));
\end{lstlisting}

\paragraph{Implementation note.} Generally speaking, the constituents
of the input jet will be reclustered with the subjet definition to
extract the subjets. The one exception is when both the clustering
giving the input jet and the subjet definition use the
Cambridge/Aachen algorithm. In that specific case one can use
exclusive subjets of the input jets and avoid reclustering its
constituents.

%% Note on the constraints on the input / extra structural stuff: do
%% we want to mention that in the previous section?

\subsection{2-Pronged taggers}
\label{sec:2prongedtaggers}


Most of the taggers that investigate the substructure of boosted jets
in order to discrinminate boosted vector bosons from QCD jets form
another big class of \emph{jet transformers}. Given an input jet, they
search for a 2-pronged structure inside it and thus naturally result
in a ``tagged'' jet with new structural information.

At the moment, we have implemented two of the taggers proposed for
boosted Higgs reconstruction: the \ttt{MassDropTagger} introduced in
\cite{BDRS} and a rest-frame \ttt{NSubjettinessTagger} from
Ref. \cite{nsubtagger}. We refer the reader to these articles for an
extensive description of the taggers and shall only give a very brief
overview in what follows. 

\subsubsection{The mass-drop tagger}

Introduced in \cite{BDRS}, this tagger starts with a fat jet obtained
...
algorithm codes as plugins for fastjet and to Salvatore Rappoccio for
...
%======================================================================
\section{Structural information for various inputs}
\label{app:structure_table}

\begin{table}[t]
\begin{tabular}{|l|c|c|c|c|c|c|}
\hline
Call & empty/input & Jet & Jet (no CS) & Constituent & Comp. Jet & Comp. Input \\
\hline
has\_associated\_cs& false   & true    &  true   & true    & false    & false   \\
associated\_cs     & NULL    &  CS     &  NULL   &  CS     & NULL     & NULL    \\
\hline                       
has\_valid\_cs     & false   & true    &  false  & true    & false    & false   \\
valid\_cs          & \throws &  CS     & \throws &  CS     & \throws  & \throws \\
\hline                       
has\_constituents  & false   & true    & true    & true    & true     & true    \\
constituents       & \throws & from CS & \throws & itself  & recurse  & pieces  \\
\hline                       
has\_pieces        & false   & true    & \throws & false   & true     & true    \\ 
pieces             & \throws & parents & \throws & empty   & pieces   & pieces  \\ 
\hline                       
has\_parents       & \throws & from CS & \throws & from CS & \throws  & \throws \\ 
has\_child         & \throws & from CS & \throws & from CS & \throws  & \throws \\ 
contains           & \throws & from CS & \throws & from CS & \throws  & \throws \\ 
\hline
\end{tabular}
\caption{Table summarising the behaviour obtained when requesting
  structural information for different kinds of
  \ttt{PseudoJet}. ``from CS'' means that the information is obtained
  from the associated \ttt{ClusterSequence}. All other entries should
  be straightforward.}\label{tab:structure}
\end{table}

Since \fastjet version 3.0, a \ttt{PseudoJet} can access information
on its structure. Typically, this means accessing clustering
information like a jet's constituents, or its pieces for a composite
jet.

There are several methods that have been added to \ttt{PseudoJet} to
allow direct access to the structure. In this appendix, we try to
present in a synthetic way what these methods will return depending on
the \ttt{PseudoJet} one have --- {\em e.g.} does a jet resulting from
a clustering have pieces, does an input particle have constituents,
...?

This has been summarised in Table \ref{tab:structure} for the most
relevant methods. For completeness, we discuss in the text below the
additional accessible information not included in the table.

All the methods accessing information related to the clustering
(\ttt{has\_partner()}, \ttt{is\_inside()},
\ttt{has\_exclusive\_subjets()}, \ttt{exclusive\_subjets()},
\ttt{n\_exclusive\_subjets()}, \ttt{exclusive\_subdmerge()}, and
\ttt{exclusive\_subdmerge\_max}) will access the assocaited cluster
sequence and throw an error if none is available (except for
\ttt{has\_exclusive\_subjets()} which would just return \ttt{false}).

For area-related calls, \ttt{has\_area()} will be \ttt{false} unless
the jet is obtained from a \ttt{ClusterSequenceArea} or is a composite
jet made from such jets. All other calls \ttt{validated\_csab()},
\ttt{area()}, \ttt{area\_error()}, \ttt{area\_4vector()}) will return
the information from the ClusterSequence (or the pieces in case of a
composite jet) and throw an error if the jet is nor associated with a
\ttt{ClusterSequenceArea}.

%======================================================================
\section{Functions of a \texttt{PseudoJet}}
\label{app:function-of-pj}

\subsection{Start of New version}

A core concept that is new to \fastjet v3.0 is that of a
\ttt{FunctionOfPseudoJet}.
%
...
It is interesting to realise that when manipulating jets, one often
needs functions that allow to obtain a result varying from a jet to
another. This could be for example the \ttt{BackgroundRescaling} (see
\ref{sec:BGE-positional}) used to parametrise the positional
dependence of the background --- and allowing one to obtain a number
associated with each jet --- or the \ttt{Filter} (see below)
associating a filtered jet to an initial jet. Generally speaking, all
these take the form
\begin{lstlisting}
...
\end{lstlisting}
that would be a function returning a result of a given type
\ttt{TOut} depending on the \ttt{PseudoJet} input.
%
For completeness and comfort, we have also added
\begin{lstlisting}
  virtual std::string description() const{ return "";}
\end{lstlisting}
that allows one to provide a description of the function and
\begin{lstlisting}
  TOut operator()(const PseudoJet &pj) const;
  vector<TOut> operator()(const vector<PseudoJet> &pjs) const;
\end{lstlisting}
equivalent to calling \ttt{apply} on one or a vector of
\ttt{PseudoJet} but in a more natural way\footnote{This construction
  where we have both \ttt{apply} and \ttt{operator()} doing the same
  thing seems redundant but it allows to redefine only \ttt{apply} in
  derived classes while, if we had only \ttt{operator()}, both
  the \ttt{PseudoJet} and \ttt{vector<PseudoJet>} version would have
  had to be overloaded.}.

This definition allows for example to pass functions of
\ttt{PseudoJet} as arguments. This is {\em e.g.} the case for the
background rescaling which would just be derived from
\ttt{FunctionOfPseudoJet<double>}. Similarly, a \ttt{Transformer} is a
function of a \ttt{PseudoJet} returning a \ttt{PseudoJet}, hence its
inheritance from \ttt{FunctionOfPseudoJet<PseudoJet>}.
