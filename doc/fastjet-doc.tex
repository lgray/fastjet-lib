...
%
This was the first jet algorithm to be implemented in \fastjet and its
...
R_{ij}^2 = (y_i - y_j)^2 + (\phi_i - \phi_j)^2$, where $y_i = \frac12
...
%
The distribution of this codebase is complemented with some limited
...
\end{lstlisting}
We have found grid spacings in the range $0.5-0.7$ to be adequate,
with lower values preferred for events that are likely to have high
multiplicities.
...
\comment{PUT A FIGURE WITH TIMING? Just have SISCone, $k_t$, C/A and
  anti-$k_t$}.
