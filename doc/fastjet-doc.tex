%% TO DO as of 2011-08-01
%%
%% - move Transformer definition (section 9.1) into appendix
%% - rewrite abstract
%% - rewrite intro
%% - check appendices
...
%, which can therefore be meaningless and should not be used in the
%presence of a rescaling \comment{should we explicitly throw?}.
...
The behaviour described above is obtained via the following
declaration of the \ttt{Transformer} base class, declared in the
\ttt{fastjet/tools/Transformer.hh} header:
\begin{lstlisting}
...
\end{lstlisting}
allows one to create a filter/trimmer that reclusters the jet's
...
Two additional constructors are also supported:
\begin{lstlisting}
...
Cambridge/Aachen algorithm is used to recluster the constituents of the jet that
the filter is applied to. 
 

%\paragraph{Usage and structure of the result.} 

A simple filter, giving the subjets obtained clustering
...
from Ref. \cite{nsubtagger}. For the three-body decays, the boosted top tagger from~\cite{Kaplan:2008ie},
\ttt{fastjet::JHTopTagger}, as well as a subjet tagger for the Cambridge-Aachen jet algorithm as
introduced in~\cite{Butterworth:2009qa}, \ttt{fastjet::CASubJetTagger}, are 
available.
...
The description of the interface to the $N$-subjettiness
tagger~\cite{nsubtagger} will follow in a later version of this
manual.
