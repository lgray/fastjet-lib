...
%% - remove references to CLHEP and KtJet (other than in footnote and
%%   to acknowledge their historical influence)
%% - maybe remove section 4, since it's really covered by section 2
%% - where do we place SR jet algs section 5? And why not move plugins closer?
%% - where do jet algs go relative to sections 3.2/3.3 JD & CS?

%----------------------------------------------------------------------
%% TO DO before a 3.0:
%% - make sure references to 3.0(alpha) are all dealt with [done]
%% - discuss UserInfo in section 3?                        [done]
%% - List of features not yet documented in section 3.5 ->
%%                        user-defined structural info appendix?
%% - section 7.5 background subtraction
%% - section 7.5 background estimation example
%% - tools(?)
%% - update spartyjet refs
%% - transfer_from_sequence
%% - transformers
%% - check all the structure stuff in the body of the documentation
%% - PJ == 0, PJ != 0
%% - join: recombiner [done] + area [done] + template versions
%% - intro to documention [at least before putting it on arXiv]
%%
%% TO DO:
%%
...
\title{FastJet 3.0beta1-devel user manual}
...
\begin{abstract}
  %% ----- FJ3THOUGHTS ----------------
  %% This should mention all main features, with a more equal weight
  %% to the various components:
  %% - C++ native fast sec.rec. & plugins for most cones
  %% - jet areas
  %% - background estimation facilities
  %% - substructure
  %% 
  
  % NEW VERSION:\\
  % \fastjet is a package that provides access to a broad range of jet
  % finding and analysis tools.
  % %
  % It includes efficient implementations of all widely-used $2\to 1$
  % sequential recombination jet finders for pp and $e^+e^-$ collisions,
  % as well as access to 3rd party jet finders through a plugin
  % mechanism, including all currently-used cone algorithms.
  % %
  % Means are provided for manipulating jet substructure, intended to
  % facilitate development of new substructure tools.
  % %
  % \fastjet also provides 
...
%% ----- FJ3THOUGHTS ----------------
%% Start with some perspective (jet finding crucial to make sense of quarks)
%% Aims 
%% - provide a reliable, efficient set of implementations
%% - facilities to allow 3rd parties to easily extend the functionality
%% - implementations of some subset of useful collider-related tools,
%%   including jet-area determination, background estimation and substructure
%% - [ability to incorporate into other frameworks easily enough]
%% This manual
%% - not an explanation of the physics, for that see other refs
%% - next section, intended to get you started quickly
%% - then systematic explanation of the full range of features
%% - also look at Doxygen and the examples
%%
%%

This note documents the \fastjet package, which provides efficient
geometrically-based C++ implementations \cite{fastjet} for the longitudinally
...
%
In version 3.0 a PseudoJet knows (where relevant) about its
...
% Such a class was not present in
...
\end{lstlisting}
A more powerful method involves passing a pointer to a derived class
...
with \ttt{reset}, the user and internal indices and user-info are
copied from the \PJ).
...
The
%
Analogous \ttt{+=}, etc., operators, are also defined.
...
\begin{lstlisting}
  ...
  vector<fastjet::PseudoJet> exclusive_jets_up_to (const int & njets) const;
\end{lstlisting}
% Note that the first of these two member functions have the same name,
% and the compiler selects the correct one based on the type of the
% arguments, as is standard in C++.
%
...
taggers.
%
Here, we outline the approach adopted, deferring more advanced usage
examples to section~\ref{sec:transformers} when we discuss
transformers and taggers.

To be able to efficiently access structural information, each \PJ has
a shared pointer to a class of type \ttt{PseudoJetStructureBase}.
%
For plain {\PJ}s the pointer is null.
%
For {\PJ}s obtained from a \CS the pointer is to a class
\ttt{ClusterSequenceStructure}, which derives from
\ttt{PseudoJetStructureBase}.
%
For {\PJ}s obtained from a \ttt{join(...)} operation, the pointer is
to a class \ttt{CompositeJetStructure}, again derived from
\ttt{PseudoJetStructureBase}.
%
...
% , in
% order to facilitate access to structural information specific to a
% given jet.
...
% %
% The particular choice we have made is intended to represent a balance
% between the usefulness of such a dichotomy and the added complication
% it brings.
%
...
\comment{The following features still need documenting. But check
  they're not already discussed later}
\begin{lstlisting}
...
\ttt{FastJet} code. 
% For the reader who is familiar with \ttt{KtJet}
...
% their use, discussed in section~\ref{sec:BackgroundEstimator}, for
...
%% Thoughts for 3.0b1 documentation
%% - keep first two paragraphs
%% - 3rd needs modifying to account for grid
%%
...
%
In the meantime an variant of the approach that has emerged is to
...
normal jet finder jets, however we have not found this to be an issue
in practice.
...
%%3.0a3 Note that we recommend that you use only the $k_t$ or Cambridge/Aachen
%%3.0a3 algorithms for calculating the UE/pileup density $\rho$ in this
%%3.0a3 way. In particular, it is important to avoid algorithms with extreme
%%3.0a3 behaviours for the areas. 
%%3.0a3 %
%%3.0a3 Specifically, one should not use anti-$k_t$ (which has many jets with
%%3.0a3 near-zero area), or SISCone (which can have jets with near-zero area,
%%3.0a3 as well as monster jets with huge areas if the overlap parameter $f$
%%3.0a3 is too small).
%%3.0a3 %
%%3.0a3 The choice of jet radius can also have an impact on the determination,
%%3.0a3 especially for events that are relatively sparse.
%%3.0a3 %
%%3.0a3 We have found that a choice in the range $R=0.4-0.6$ is generally
%%3.0a3 adequate~\cite{cs,Cacciari:2009dp} with a preference for larger values
%%3.0a3 if the events are relatively sparse and smaller values if you expect
%%3.0a3 your events to have many hard jets.
...
with $|y|<4.5$), together with a jet definition and an area definition
(typically, an active area with explicit ghosts is
recommended\footnote{With the $k_t$ algorithm, when the selector is
  geometrical and selects only jets well within the range of event
  particles, one may also use a Voronoi area (\ttt{effective\_Rfact =
    0.9} is recommended), which has the advantage of being
  deterministic and faster than ghosted areas.}).
%
...
\ttt{BackgroundEstimatorBase} class.
...
%%FJ3.0a3 Two elements are needed for FastJet's background estimation: a cluster
%%FJ3.0a3 sequence with area information and a \ttt{Selector} to specify the
%%FJ3.0a3 subset of jets that will be used for the background estimation.
%%FJ3.0a3 %
%%FJ3.0a3 The class that provides background estimation facilities is
%%FJ3.0a3 \ttt{JetMedianBackgroundEstimator}. The simplest way to construct it is:
%%FJ3.0a3 \begin{lstlisting}
%%FJ3.0a3    #include "fastjet/tools/JetMedianBackgroundEstimator.hh"
%%FJ3.0a3    // ...
%%FJ3.0a3 
%%FJ3.0a3    // constructor for a JetMedianBackgroundEstimator that will use the
%%FJ3.0a3    // inclusive jets from csa that pass the cuts specified by
%%FJ3.0a3    // the selector in order to estimate the background density
%%FJ3.0a3    JetMedianBackgroundEstimator(const ClusterSequenceAreaBase & csa,
%%FJ3.0a3                        const Selector & selector);
%%FJ3.0a3 \end{lstlisting}
%%FJ3.0a3 If, for example, one wants to use all jets within $|y|<4.5$ to
%%FJ3.0a3 estimate the background, one should choose \ttt{selector =
%%FJ3.0a3   SelectorAbsRapMax(4.5)}. The background density $\rho$ and its
%%FJ3.0a3 fluctuations $\sigma$ can then be accessed through the functions:
%%FJ3.0a3 \begin{lstlisting}
%%FJ3.0a3    // the median of ($p_t$/area) for jets that pass the selection cut, 
%%FJ3.0a3    // making use also of information on empty area in the event
%%FJ3.0a3    double rho()   const; 
%%FJ3.0a3 
%%FJ3.0a3    // an estimate of the fluctuations in the $p_t$ density per unit $\smash{\sqrt{A}}$,
%%FJ3.0a3    // which is obtained from the 1-sigma half-width of the distribution of pt/A.
%%FJ3.0a3    // To be precise it is defined such that a fraction (1-0.6827)/2 of the jets
%%FJ3.0a3    // (including empty jets) have $p_t/A < \rho - \sigma \sqrt{\langle A \rangle}$
%%FJ3.0a3    double sigma() const; 
%%FJ3.0a3 \end{lstlisting}
%%FJ3.0a3 Note that $\rho$ and $\sigma$ determinations count empty area within
%%FJ3.0a3 the selected region as consisting of jets of zero $p_t$.
%%FJ3.0a3 %
%%FJ3.0a3 Thus (roughly speaking), if more that half of the area covered by the
%%FJ3.0a3 selector is empty the median estimate for $\rho$ will be zero.
...
alternative constructors to \ttt{JetMedianBackgroundEstimator}.
%
...
multiple contexts and it is more efficient to perform it just once:
\begin{lstlisting}
  // set just the selector, leaving all other information to be provided later
...
%%TMP_REMOVED Finally, there are cases where the user may wish to determine medians
%%TMP_REMOVED of quantities other than $p_t/A$. 
%%TMP_REMOVED %
%%TMP_REMOVED This may arise in estimating the hadron-mass contribution to pileup
%%TMP_REMOVED contamination of jet masses, or in the determination of moments of
%%TMP_REMOVED fragmentation functions.
%%TMP_REMOVED %
%%TMP_REMOVED For this purpose, the user may use the following call
%%TMP_REMOVED \begin{lstlisting}
%%TMP_REMOVED   void set_jet_density_class(const FunctionOfPseudoJet<double> * jet_density_class);
%%TMP_REMOVED \end{lstlisting}
%%TMP_REMOVED For each jet that passes the selection, \ttt{jet\_density\_class}'s
%%TMP_REMOVED \ttt{FunctionOfPseudoJet<double>::operator(PseudoJet \& jet)} function
%%TMP_REMOVED is called\footnote{As explained in Appendix \ref{app:function-of-pj}
%%TMP_REMOVED   describing functions of \ttt{PseudoJet}, user-defined density
%%TMP_REMOVED   classes derived from \ttt{FunctionOfPseudoJet<double>} should
%%TMP_REMOVED   overload \ttt{FunctionOfPseudoJet<double>::result()}.} and it returns
%%TMP_REMOVED the quantity that substitutes $p_t/A$.
%
...
\ttt{GridMedianBackgroundEstimator} classes are new to FastJet 3.0.
%
...
%
This offset is no longer present with the 3.0 ghost placement.
%
...
The \ttt{Subtractor} class is defined in
...
%
Note that \ttt{Subtractor} derives from
...
%   double median_pt_per_unt_area(const RangeDefinition & range) const;
% 
...
See \ttt{example/07-subtraction.cc}.
...
%
As of version 3.0, \fastjet provides a common interface for such
...
we call \ttt{Transformer}s. 
%
We then describe some that have already been implemented. 
%
Finally, we outline some of the techniques that can help when creating
new transformers.


\subsection{Generic considerations about \texttt{Transformer}s}
\label{sec:transformers-generic}

The common interface to all the transformers, declared in the
\ttt{fastjet/tools/Transformer.hh} header, is the following:
\begin{lstlisting}
...
\end{lstlisting}
It is essentially the same as that of the
\ttt{FunctionOfPseudoJet<PseudoJet>} from which it derives (cf.\
appendix~\ref{app:function-of-pj}).
%
While the action of a \ttt{Transformer} is to be implemented in the
\ttt{result(...)} member function,
%
typically it will be accessed through the \ttt{operator()} function:
\begin{lstlisting}
  /// just wraps result()
...
\end{lstlisting}
i.e.\ if we have a class \ttt{MyTransformer} derived from
\ttt{Transformer} then it can be used as follows:
\begin{lstlisting}
...
%
Using \ttt{jet.has\_structure\_of<MyTransformer>()}, it is possible to
check if \ttt{jet} is compatible with the structure provided by
\ttt{MyTransformer}.
...
\subsection{Jet Filtering and Trimming using \texttt{Filter}}
\label{sec:filtering}
...
%
The \ttt{Filter} class, defined in \ttt{fastjet/tools/Filter.hh},
handles both operations. Its constructor
\begin{lstlisting}
...
\end{lstlisting}
allows one to specify a filter/trimmer that reclusters the jet's
constituents with the jet definition \ttt{subjet\_def} and then
...
distance between the parents of the jet). In both cases, the
Cambridge/Aachen algorithm is used.
...
\begin{lstlisting}
  ...
  // the original jet before filtering
...
fraction of the input jet, is done using
\begin{lstlisting}
...
%
Usually, the subjets are obtained simply by clustering the input jet's
constistuents with the subjet definition.
% 
% constituents of the input jet are simply reclustered with
% the subjet definition in order to extract the subjets.
% 
% 
% Usually, the constituents of the input jet are simply reclustered with
% the subjet definition in order to extract the subjets.
%
However, when the input jet was obtained with the Cambridge/Aachen
...
\subsection{2-pronged and 3-pronged taggers}
\label{sec:2prongedtaggers}

A number of the taggers developed to distinguish 2- or 3-pronged
decays of massive objects from plain QCD jets naturally fall into the
...
At the moment, we have implemented two of the taggers proposed for the
reconstruction of boosted two-body decays: the \ttt{MassDropTagger}
introduced in \cite{BDRS} and a rest-frame \ttt{NSubjettinessTagger}
from Ref. \cite{nsubtagger}. We refer the reader to these articles for
an extensive description of the taggers and shall only give a very
brief overview in what follows.
...
\begin{lstlisting}
   MassDropTagger(double $\mu$, double $y_\text{cut}$);
\end{lstlisting}
...
\end{lstlisting}
%   Filter filter = Filter(Rfilt, SelectorNHardest(3));
...
The Johns Hopkins top tagger~\cite{Kaplan:2008ie} is available by
including \ttt{fastjet/tools/JHTopTagger.hh}.
%
...
%
The \ttt{pieces()} function will return 3 or 4 pieces, where the first
two are the two prongs of the $W$, while the third (and occasionally a
softer fourth) is(are) the remaining subjet(s).
...
\begin{lstlisting}
     ...
     SimpleFilterStructure * structure = 
                       dynamic_cast<SimpleFilterStructure *>(result.structure_non_const_ptr());
...
% In many cases transformers simply return a subjet from an existing \CS
% or alternatively join some number of subjets and return the result.
% %
% Two helpers are available to help the user take care of the structural
% information of the resulting jet while also adding additional
% information, for example about the kinematic variables such as $\mu$
% and $y$ in the \ttt{MassDropTagger} above.
% 
% The first helper is a templated version of \ttt{join}:
% \begin{lstlisting}
%    template<class DerivedFromCompositeJet> PseudoJet join([some jets]);
% \end{lstlisting}
% It works exactly as the usual \ttt{join} function,
% section~\ref{sec:composite-jet}, except that the class associated with
% the jet's underlying structure can now be chosen by the user to be
% \ttt{DerivedFromCompositeJet}.
% %
% Specifically, it can be any class derived from
% \ttt{CompositeJetStructure}, which is the class performs the work of
% storing the information about the pieces of the resulting jet for the
% usual \ttt{join} function.
...
\paragraph{How plugins work.}
%
The base class from which plugins derive have the following structure:
\begin{lstlisting}
...
%% In cases where the plugin has been created with a \ttt{new} statement
%% and the user does not wish to manage the deletion of the corresponding
%% memory when the \ttt{JetDefinition} (and any copies) using the
%% plugin goes out of scope, then the user may wish to call the
%% \ttt{delete\_plugin\_when\_unused()} function, which tells the
%% \ttt{JetDefinition} to acquire ownership of the pointer to the
%% plugin and delete it when it is no longer needed.
%
...
ClusterSequence (or the pieces in case of a composite jet) and throw
...
A concept that is new to \fastjet v3.0 is that of a
...
% \subsection{Old version of this documentation}
...
{\PseudoJet}s.\footnote{Having \ttt{result(...)} and
  \ttt{operator(...)} doing the same thing may seems redundant,
