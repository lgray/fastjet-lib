...
$k_t$ algorithm (below,
section~\ref{sec:areas-and-background-subtraction}): with the choice
$\ttt{effective\_Rfact}\simeq0.9$ it provides an acceptable
...
\label{sec:areas-and-background-subtraction}
...
of interest. After selecting a phase space region over which to
analyse the jets, one can determine the average level $rho$
(transverse momentum per unit area) of the background, and also
subtract it from the hard jets.
%
A difficulty in estimating the background is that it is difficult to
introduce a robust criterion to distinguish ``background'' jets from
hard jets. 
%
...
event and then taking the median of the distribution as an estimate of
$\rho$, as proposed in~\cite{cs} and studied in detail also
in~\cite{Cacciari:2009dp}.
%
This is insensitive to the presence of even very hard jets, and avoids
any need for introducing a $p_t$ scale to distinguish hard and
background jets.
...
behaviours for the areas. For example, one should not use anti-$k_t$
(which has many jets with near-zero area), nor SISCone (which can have
jets with near-zero area, as well as monster jets with huge areas if
the overlap parameter $f$ is too small).
...
\end{lstlisting}
If one wants to use all jets within $|y|<4.5$ to estimate the
background, then one would choose \ttt{selector =
  SelectorAbsRapMax(4.5)} and then access the background density
$\rho$ and its fluctuations $\sigma$ 
...
\begin{lstlisting}
   ...
   // defined as the 1-sigma half-width of the distribution of $p_t$/A, obtained
   // by looking for the value $\sigma$ such that a fraction (1-0.6827)/2 of the jets
   // (including empty jets) have $p_t/A < \rho - \sigma \smash{\sqrt{\langle A \rangle}}$
   double sigma() const; 
\end{lstlisting}
The use of information on empty area is crucial if the method is to
correctly return $\rho = 0$ in events where there are only a handful
of jets, i.e. there is no background at all in most of the event.
%
\comment{carry on here about empty areas -- and check what we already have}
...
this information is not correctly accounted for.
%
...
half-width $\Delta y$ (which might be of order $1$).
%
...
selector on the specified jet. Then the jets in \ttt{csa} that pass
the cut specified by this newly positioned \ttt{selector} are used to
estimate $\rho$ or $\sigma$.\footnote{If the selector is not
  modified by the reference jet, then these calls give identical
  results to the plain \ttt{rho()} and \ttt{sigma()} calls, unless a
...
%
Therefore this method is adequate for dijet events in $pp$ or PbPb
...
as
\begin{lstlisting}
   selector = SelectorStrip($\Delta y$) * (!SelectorNHardest(4))
\end{lstlisting}
which removes the 4 hardest jets globally and then, of the remainder,
takes the ones within the strip. This is however not always very
effective.\footnote{If you use non-geometric selectors in determining
...
\comment{Carry on here(?) with information about mean area, empty
  area, etc.?}
