...
divergences in perturbative QCD and
%
...
\end{lstlisting}
Each algorith is described in detail in section~\ref{sec:native-algs}.
%
...
% \end{lstlisting}
%
Details are to be found in sections~\ref{sec:genkt}--\ref{sec:ee-algs}.
%
...
momenta once each jet's consistuents have been identified.
%
...
%Instead of the seed threshold used in the CDF cones, 
...
finding. 
%
Such a threshold and the pre-clustering are not included in the code
...
as being applied to calorimeter towers in data and Monte Carlo studies
...
\fastjet under the terms of the CPL license.
% %
% The PxCone algorithm is also described in ref.~\cite{Seymour:2006vv}.
...
all area definitions tend to the same value~\cite{CSSAreas}. 
%% GPS: To some extent a more precise recommendation comes later, so
%% avoid it here.
%%
% It might
...
To access the areas of the jets, the user is exposed to two main
classes:
\begin{lstlisting}
...
%----------------------------------------------------------------------
\subsection{\tt fastjet::AreaDefinition}
...
% follows:
% \begin{lstlisting}
%   /// returns a description of the current area definition
%   std::string description() const ;
% 
%   /// returns info about the type of area being used by this defn
%   AreaType area_type() const ;
% 
%   /// returns a reference to the ghosted area spec (where relevant)
%   const GhostedAreaSpec  & ghost_spec()  const ;
% 
%   /// returns a reference to the voronoi area spec (where relevant)
...
% One may choose to place ghosts well beyond the acceptance for
% particles so that jet-areas retain their geometric meaning even for
% jets near the edge of the particle acceptance. In this case 
% 
%  --- typically jet areas will be reliable for jets
% up to rapidity $|y| \simeq \ttt{ghost\_maxrap} - R$. 
...
$\ttt{repeat}=1$.\footnote{%
  ...
  % 
  Each ghosts's $p_t$ is distributed randomly in the range $(1 \pm \frac12
...
% The reason why \fastjet adds random fluctuations to the ghosts, giving
% each one a different $p_t$ and a position slightly away from a grid
% vertex is to the huge clustering degeneracies that would arise if all
% the ghosts had identical $p_t$'s and each were placed at a grid vertex.
% 
% \fastjet places ghosts close to the vertices of a rapidity and azimuth
% grid but not exactly on them.
% %
% The reason for this is that many jet algorithms have degeneracies in
% the clustering order if particles are placed on an exact grid.
% %
% The use of an approximate grid breaks those degeneracies.
% %
% Each ghost's position differs from an exact grid vertex by a random
% amount distributed uniformly in the range $\pm \frac12
% \ttt{grid\_scatter}$ times the grid spacing in both the rapidity and
% azimuth directions.
% %
% Each ghosts's $p_t$ is distributed randomly in the range $(1 \pm 0.5
% \texttt{pt\_scatter})$ times \ttt{mean\_ghost\_pt}.
% %
% The values for the scatter and mean ghost $p_t$ can nearly always be
...
usual, with a \ttt{description()} member function.
% follows:
% \begin{lstlisting}
%   std::string description() const;
% \end{lstlisting}
...
%----------------------------------------------------------------------
\subsection{\tt fastjet::ClusterSequenceArea}
