...
\title{\sf FastJet user manual%
  \\ \large (for version
  3.1.0-devel%  %%%-VERSION-NUMBER (do not remove this comment; used
...
to directly perform the clustering and access the inclusive jets thorugh the
definition of a \ttt{()} operator:
\begin{lstlisting}
...
\end{lstlisting}
returns the transverse momentum-ordered list of all the inclusive jets given by
clustering with the anti-$k_t$ algorithm with radius parameter $R=0.4$.
...
control over the which tiles a grid-based estimator uses.
%
...
masses.
%
In some situations those masses are ignored and set to zero, e.g.\
...
%
However, if hadron masses are kept non-zero, then
...
\end{equation}
where $\rho_m$ tracks the massive component of the energy flow. 
%
...
\end{equation}
By default, as of \fastjet 3.1, grid and jet-median background
...
To avoid changes in results relative to earlier versions, by default
...
default so that it is automatically switched on.
%
...
%
However, for most applications a sensible behaviour is to adjust the
...
its \ttt{set\_safe(true)} function (available since v.3.1).
%
...
%
There one will also find the \ttt{GenericSubtractor} contrib, which
code for area-based subtraction of jet shapes and other generic
observables as discussed in\cite{Soyez:2012hv,Cacciari:2012mu}.
...
%
In that case the can provide a user-defined a selector
...
%----------------------------------------------------------------------
\subsection{FastJet Contrib}

The \fastjet update schedule is geared towards providing stability.
Typically, a minor (X.Y) release is made every two or three years, and
a new minor or major release may only become widely adopted (notably
by the experiments) a further year or two down the line.
%
...
To accomodate this, part way through the 3.1 development cycle, in
...
on the \href{http://fastjet.hepforge.org/contrib/}{\fjcontrib}{ web pages~\cite{fjcontrib}.
%
...
\ttt{N2MHTLazy9AntiKtSeparateGhosts}, intended specifcally for area
calculations with the anti-$k_t$ algorithm (though it can also be used
for passive area calculations for other algorithms).
...
the fact that one searches for neighbours neighbours in a smaller set
of tiles.
%
...
rapidities up to $|y|<5$.\footnote{The optimal transtion to the
...
\begin{figure}[tp]
  ...
  \caption{
    ...
    $N$ hardest particles. \comment{still correct? Rapidity limits?}
    %%
...
algorithm in \fastjet 3.0.1 and also for SISCone ($R=0.4$ only).
%
...
%
The speed improvements from the the strategies introduced in version
...
strategy could be further extended to the \ttt{Lazy25} case.
%
