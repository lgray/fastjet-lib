...
is called\footnote{As explained in Appendix \ref{app:function-of-pj}
  ...
  overload \ttt{FunctionOfPseudoJet<double>::result()}.} and it returns
...
%
If \ttt{jet.perp() < bge.rho(jet)*jet.area\_4vector().perp()}, then the
subtractor instead returns a jet with zero 4-momentum.
%
...
See \ttt{example/07-subtraction-new.cc}.
...
A second way of extending of adding information to an existing
structure class is to ``wrap'' it. This can be done with the help of
the \ttt{WrappedStructure} class. 
\begin{lstlisting}
   Class MyStructure: public WrappedStructure {
     MyStructure(const SharedPtr<PseudoJetStructureBase> & structure) :
