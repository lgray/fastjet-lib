...
\title{FastJet 3.0alpha1 user manual}
...
\ttt{PseudoJet::ExtraInfo} is discussed in
appendix~\ref{app:extra-info}. 
...
In addition there are also some \emph{relocatable} selectors. 
%
These have been developed in anticipation of usefulness for background
estimation, where one wishes to select a fixed-shape (in $y,\phi$)
neighbourhood of first one jet, then another jet, and so forth.
%
For example one can define 
\begin{lstlisting}
  Selector sel = SelectorCircle(1.0);
\end{lstlisting}
Then if one is interested in the subset of \ttt{jets} neighbourhood of
...
\end{lstlisting}
Using a relocatable selector before it has ever been relocated will
cause an exception to be thrown.
%
The other relocatable selectors that are available by default are
\ttt{SelectorStrip}, \ttt{SelectorDoughnut} and
\ttt{SelectorRectangle}.
...
%======================================================================
\section{Extra Info in PseudoJets}
\label{app:extra-info}

One method for associating extra information with a \ttt{PseudoJet} is
...
A second method for supplementing a \PJ with extra information is for
the user to derive a class from \ttt{PseudoJet::ExtraInfo} and
...
\ttt{set\_extra\_info(...)} transfers ownership of the pointer to the
\ttt{PseudoJet}. This is achieved with the help of a shared
pointer. Copies of the \ttt{PseudoJet} will point to the same
\ttt{extra\_info}. When the \ttt{PseudoJet} and all its copies go out
of scope the \ttt{extra\_info} will be deleted.
%
Since nearly all practical uses of \ttt{extra\_info} will require it
to be cast to the relevant derived class of  \ttt{ExtraInfo}, we also
provide the following member function for convenience:
\begin{lstlisting}
  template<class L> const L & extra_info_cast() const;
\end{lstlisting}
...
The user may wonder why we have used shared pointers internally (i.e.\ have
ownership transferred to the \ttt{PseudoJet}) rather than normal
pointers. 
%
An example use case where the difference is important is if, for
example, one wishes to write a \ttt{Recombiner} that sets the
\ttt{extra\_info} in the recombined \PJ.. 
%
...
\end{lstlisting}
%
It behaves almost identically to the \ttt{C++0x} \ttt{shared\_ptr},
%
though some internal details are different.
%
The end-user should not usually need to manipulate the
\ttt{SharedPtr}, though the \ttt{SharedPtr} to \ttt{extra\_info} is
accessible through \PJ's \ttt{extra\_info\_shared()} member.
