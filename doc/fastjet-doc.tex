...
Since a jet is constituted by only a finite number of particles, one needs a
specific definition in order to make its {\sl area}  (i.e. the surface in the
$\eta$-$\phi$ plane over which it extends) an unambiguous concept. 

\ttt{FastJet} implements what is called an {\sl active} area.
It is determined numerically by superimposing to the initial particles provided
by the user a large number of extremely soft ``ghost'' particles, distributed
on a uniform grid (with small random fluctuations to avoid degeneracies) in
$\eta$ and $\phi$, which are then clusterd together with the real ones. The
number of ghosts which belong to a jets gives a measure of its area.

Alternative definitions of areas will be implemented in future versions.

\subsection{fastjet::ActiveAreaSpec}
The user must first of all create an object that specifies how he defines
the (active) area. This is done via the class \ttt{fastjet::ActiveAreaSpec} 
...
\end{verbatim}
\ttt{ghost\_etamax} defines the maximum rapidity up to which ghosts are
generated. \ttt{ghost\_area} is the area of a single ghosts. The smaller this
number the slower the program will run, but the more accurate the resulting
areas will be. \ttt{repeat} represents the number of times
...
\ttt{grid\_scatter} and \ttt{kt\_scatter} are fractional random fluctuations of
the position of the ghosts on the $\eta$-$\phi$ grid and of their transverse
momentum.  \ttt{mean\_ghost\_kt} is the average transverse momentum of the
ghosts.


\subsection{fastjet::ClusterSequenceWithArea}
Base class that sets the interface for extensions 
of \ttt{fastjet::ClusterSequence} that provide information about 
the area of each jet. It inherits from \ttt{fastjet::ClusterSequence}.
...
\subsection{fastjet::ClusterSequenceActiveArea}

Main interfece for the active area determination. The user must define an object
of this type in order to access the areas of the jets. Its constructor is
\begin{verbatim}
...
This class inherits from \ttt{fastjet::ClusterSequenceWithArea}. In addition it
includes some methods for extracting information about the estimated background
in the clustered event. These methods might be ameliorated and refined, and 
migrate to the base class, in the future.
\begin{verbatim}
...
pileup, while \ttt{parabolic\_pt\_per\_unit\_area} has been found to provide a
...
\ttt{mean\_pt\_strategies} are options of \ttt{pt\_per\_unit\_area}. They dictate
...
\begin{itemize}
\item[]\ttt{old\_median} - The median value of the set $\{{k_t}_i/area_i\}$, where
$i$ runs over all jets found in the event.
\item[]\ttt{median} - As previous one, but $i$ now runs only over the `real' jets (i.e.
exluding those made up exclusively of ghosts). This definition is the default
behaviour, and will coincide with the previous one for densely populated events. 
\item[]\ttt{pttot\_over\_areatot} - The total pt over the total area, i.e. $\sum_i
{k_t}_i/\sum_i area_i$, with $i$ running over all jets, including the only-ghost
ones, if present.
\item[]\ttt{pttot\_over\_areatot\_cut} - As previous one, but now the sum runs only on
the jets whose $k_t/area$ ratio is smaller than \ttt{range} times the median
defined by the option \ttt{median}. The rationale behind this option is of
course that the hard jets tend to have a much larger transverse momentum over
area ratio, and this cut excludes them from the background estimation.
\item[]\ttt{mean\_ratio\_cut} - Cut as in previous option, but now the mean 
ratio is calculated, $(1/\#~jets~in~sum) \sum_i {k_t}_i/area_i$.
\item[]\ttt{play} - The function returns 0, but writes out a number of different
quantities and results for various optionis. Mainly used for testing and
development. Might disappear in future versions.
\end{itemize}

\ttt{parabolic\_pt\_per\_unit\_area} has no strategy optinos, and simply
returns the coefficents $a$ and $b$ of the least square fit of all the
$\{{k_t}_i/area_i\} <$ \ttt{exclude\_above}  to the parabolic shape $a +
