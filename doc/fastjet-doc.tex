...
\title{\sf FastJet user manual%
  \\ \large (for version
  3.2.2-devel%  %%%-VERSION-NUMBER (do not remove this comment; used
...
As of version 3.3 \fastjet ships with a Python interface, whose
compilation is enabled at configure time.
%
It includes access to much of the functionality in FastJet, except
plugins and certain features are difficult to map to Python.
%
In particular templated types, nested classes and user-derived classes
(e.g.\ a user-defined class derived from \ttt{UserInfoBase}) are not
currently supported.
%
The python interface was created automatically, using
SWIG~\cite{swig}.
%
...
\item For many objects that provide definictions of some kind, a
