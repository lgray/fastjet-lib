...
\title{\sf FastJet user manual%
  \\ \large (for version
  3.3.5-devel%  %%%-VERSION-NUMBER (do not remove this comment; used
             %  for automatic version-number changes; version number must
             %  stay one a single line)
  )
}
\author{Matteo Cacciari,$^{1,2}$ Gavin P. Salam$^{3,4,}$\footnote{On
    leave from CERN, Theoretical Physics Department, Geneva,
    Switzerland and from CNRS, UMR 7589, LPTHE, F-75005, Paris, France.}\; and Gregory Soyez$^{5}$\\[10pt]
  \normalsize
...
threads, for reasons related to the internal caching of rapidity and
azimuth.
%
The \ttt{SharedPtr} and objects that rely on them are also not
thread-safe.
...
%
By default the generator is a static and common across all threads.
%
...
for that set of ghosts can be retrieved with \comment{rename \ttt{get\_random\_seeds\_used}?}
\begin{lstlisting}
  vector<int> seeds;
  csa.area_def().ghost_spec().get_used_random_seed(seeds);
\end{lstlisting}
