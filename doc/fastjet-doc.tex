...
\title{FastJet 3.0.0alpha1 user manual}
...
deleted, or in any other way rendered invalid.
%
...
\end{lstlisting}
Using a relocatable range before it has ever been relocated will
...
serve as useful hints in some contexts (for example, in anticipation of
their future use for background estimation).
%
Their rapidity extent is accessible through a
\ttt{get\_rapidity\_extent(rapmin,rapmax)} call.
%
...
returned are the largest (positive and negative) numbers that can be
represented as doubles.

\comment{carry on with areas}
