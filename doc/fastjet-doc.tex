...
\ttt{jet1}, one performs the following operations:
\begin{lstlisting}
  sel.relocate(jet1);
...
\comment{to come}:
\begin{itemize}
\item relocatable ones
\item do we list all implemented selectors? (Selector{Type}{Var}),
  plus some exceptions
\item areas?
\item do we tell users how to write their own Selectors
\end{itemize}
