...
For the generalised $k_t$ algorithm an extra parameter is needed, and
the following constructor should then be used
%
\begin{lstlisting}
...
\end{table}
%
All strategies apply equally well to the any of the internally
implemented jet algorithms, except \ttt{NlnNCam}, which being based on a
...
%
Note that $N \ln N$ performance of the \texttt{NlnN} strategy is an
\emph{expected} result, and while it holds in practice for the $k_t$
and Cambridge algorithms, for anti-$k_t$ and generalised-$k_t$ with
$p<0$, hub-and-spoke (bicycle-wheel!) type configurations emerge
dynamically during the clustering and these break the conditions
needed for the expected result to hold (this however has a significant
impact only for $N \gtrsim 10^5$).
