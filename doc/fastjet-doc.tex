...
\title{FastJet 2.4 [deve] user manual}
...
Version 2.3 includes the new version 1.3.0 of SISCone,
...
%......................................................................
\subsection{Generalised $\boldsymbol{k_t}$ algorithm for
  $\boldsymbol{e^+e^-}$ collisions}
\label{sec:ee-algs}
...
\begin{align}
  d_{ij} &= \min(E_i^{2p}, E_j^{2p}) \frac{2(1- \cos \theta_{ij})}{R^2}\,,\\
  d_{iB} &= E_i^{2p}\,,
\end{align}
\end{subequations}
...
For $R > 2$, the $d_{iB}$ will be smallest only if the event
...
For values of $R \le 2$ in eq.~(\ref{eq:dij_eegenkt}), the generalised
$e^+e^-$ $k_t$ algorithm behaves in analogy the $pp$ algorithms: when
an object is at an angle $\theta_{iX} >\arccos (1 - R^2/2)$ from all
...
sphere, with opening half-angle $\arccos (1 - R^2/2)$.
...
%
\begin{lstlisting}
  fastjet::JetDefinition(fastjet::JetAlgorithm jet_algorithm = kt_algorithm,
                         double R = 1.0,
...
%
If the user chooses the \ttt{ee\_kt\_algorithm} (Durham), then the $R$
value is ignored since that algorithm has no such parameter.
%
...
For the generalised $k_t$ algorithm and its $e^+e^-$ version an extra
parameter is needed, and the following constructor should then be used
%
\begin{lstlisting}
...
\end{lstlisting}
%
If the earlier constructor is used, then $p$ is set to 0.
...
\url{http://www.cgal.org/}. Under linux, after compilation and
installation, the user will be encouraged to set an environment
variable \ttt{CGAL\_MAKEFILE}, which points to the Makefile generated
by \ttt{CGAL} at install time, which contains various definitions of
locations of include files. The user may specify the location of this
file to \fastjet either through the above environment variable, or
with the \verb|--with-cgalmakefile=...| configuration option.
...
We are grateful to CDF, Joey Huston and Mike Seymour for providing
