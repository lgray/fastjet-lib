...
%
In the meantime the plugin feature provides a similar functionality.
...
$y$-$\phi$ plane over which it extends) an unambiguous
...
%
The \verb:overlap_threshold: is the fraction of overlapping transverse
mass ($m_t^2 = p_t^2 + m^2$) above which two protojets are merged in a
Tevatron Run~II type \cite{RunII-jet-physics} split--merge procedure.
The radius and overlap parameters are a common feature of most modern
cone algorithms. Because some event particles are not to be found in
any stable cone \cite{EHT}, SISCone can carry out multiple stable-cone
...
highest pass that gives no new stable cones. Setting the
...
A final comment concerns the \verb:split_merge_on_transverse_mass:
parameter. If \verb:true:, split--merge uses the transverse mass as
its ordering variable. If \verb:false:, it uses transverse momentum
--- the latter leads to infrared unsafety and so is to be avoided in
general.  The option of using it is nevertheless kept for historical
comparison purposes, since many previous cone algorithms used $p_t$ as
