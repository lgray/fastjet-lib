...
\ttt{fastjet::RangeDefinition} contains three virtual functions,
\begin{verbatim}
  /// return bool according to whether the jet is within the given range
  virtual bool is_in_range(const PseudoJet & jet) const ;
...
\end{verbatim}
These function can be overloaded by the user in a derived class, where a
...
extends up to a given distance $d$. 
...
\ttt{ClusterSequenceArea::median\_pt\_per\_unit\_area()} in such cases
...
public:
  ...
  /// return total area of range
  virtual inline double area() const { return fastjet::pi*_distance*_distance; }
  
  /// return bool according to whether jet is in range
  virtual inline bool is_in_range(const fastjet::PseudoJet & jet) const {
...
private:
  ...
  fastjet::RangeDefinition _range;
};
\end{verbatim}
...
standalone
  code available from \url{http://projects.hepforge.org/siscone}.
