...
\begin{verbatim}
  ...
  double area (const PseudoJet & jet) const ;

  /// return the error (uncertainty) associated with the determination
  /// of the area of the jet;
  double area_error (const PseudoJet & jet) const ;
...
% The \ttt{repeat} variable indicates the number of times
% the area of each jet is calculated before a result is given as an average
% of the various runs. If \ttt{repeat} $ > 1$ a statistical
% uncertainty, given by $\sigma/\sqrt{\ttt{repeat}-1}$, is also
% given.
...
particles are not included in the the jets returned to the user.
...
\item[]\ttt{old\_median} - For each jet $i$ one defines the quantity
...
\item[]\ttt{median} - As above, but where the median is taken over
  \emph{all} jets, including purely ghost jets. This definition
  coincides the previous one for densely populated events, and
...
quantities and results for various optionis. Mainly used for testing and
...
returns the coefficents $a$ and $b$ of the least square fit of all the
