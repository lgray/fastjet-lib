...
\item Account for the rapidity dependence of pileup, either with the
  rescaling method (section~\ref{sec:rescaled-bkgd-estimation}) or via
  a local background estimator (section
  \ref{sec:local-bkgd-estimation}) or the use of regions
  (section~\ref{sec:region-bkgd-estimation}).
  % 
  ...
  option in the case of heavy-ion collisions.
...
\item If you are interested in jet masses, and wish to use non-zero
  ...
  masses, and consider calling the subtractor's \ttt{set\_safe()}
  option.
