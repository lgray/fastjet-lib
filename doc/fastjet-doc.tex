...
and is an \texttt{enum} of type \texttt{Strategy} with
...
To run the jet clustering, create a \texttt{ClusterSequence}
...
A number of example programs are distributed with \fastjet in the \ttt{example/}
directory. 
...
A simple example of a working code is given below.
...
int main (int argc, char ** argv) {
  ...
  while (cin >> px >> py >> pz >> E) {
    // push PseudoJet of (px,py,pz,E) on back of input_particles
...
Assuming that \ttt{fastjet-config} is in the path, this can be built using
\begin{lstlisting}
g++ example.cc -o example `fastjet-config --libs --cxxflags`
\end{lstlisting}
...
%section~\ref{sec:ex-prog-sub}. \comment{Check it before release...?}
...
%
The \ttt{fastjet::Filter} class, defined in \ttt{fastjet/tools/Filter.hh},
...
reconstruction of boosted two-body decays: the \ttt{fastjet::MassDropTagger}
introduced in \cite{BDRS} and a rest-frame \ttt{fastjet::NSubjettinessTagger}
from Ref. \cite{nsubtagger}. The boosted top tagger from~\cite{Kaplan:2008ie},
\ttt{fastjet::JHTopTagger}, as well as a subjet tagger for the Cambridge-Aachen jet algorithm as
introduced in~\cite{Butterworth:2009qa}, \ttt{fastjet::CASubJetTagger}, are also
available.
We refer the reader to the original papers for
...
via its user index (section~\ref{sec:PseudoJet}). This is
