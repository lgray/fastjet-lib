...
\ref{app:function-of-pj}). Then, transformers are likely to bring new
structural information with the returned result. For a given
transformer, say \ttt{MyTransformer}, the new information that is not
directly accessible from \ttt{PseudoJet} (like its \ttt{constituents},
\ttt{pieces} or \ttt{area} when they are available), coulb be accessed
through
\begin{lstlisting}
...
if \ttt{jet} is compatible with the information provided by
\ttt{MyTransformer}.
...
non-zero, each of the subjets will be subtracted prior to the
