...
\title{FastJet 3.0.1-devel user manual}
...
field, for example~\cite{Moretti:1998qx,Ellis:2007ib,Salam:2009jx,Ali:2010tw}.
%
...
criterion is reached. The differences between them lie in the choice
of distance measures and stopping criterion.
%
Generally, the distance measure is related to the structure of
divergences in perturbative QCD.
%
Cone algorithms instead put together particles within specific conical
angular regions, notably such that the momentum sum of the particles
contained in a given cone coincides with the cone axis (a ``stable
cone'').
%
...
the strategy chosen to search for the stable cones (e.g.\ whether
iterative or exhaustive) and the procedure used to deal with cases
where the same particle is to be found in multiple stable cones (e.g.\
via split--merge procedures).
...
manual, let us consider the $k_t$ algorithm for hadron
colliders~\cite{ktexcl,ktincl}.
%
...
%
% As concerns sequential recombination algorithms, the main challenge
% that would be faced by a user wishing to write their own
% implementation is one of computational efficiency.
%
...
\end{equation}
where $p_{ti}$ is the transverse momentum of particle $i$ and $\Delta
R_{ij}^2 = (y_i - y_j)^2 + (\phi_i - \phi_j)^2$, with $y_i = \frac12
\ln \frac{E_i + p_{zi}}{E_i - p_{zi}}$ and $\phi_i$ are respectively
$i$'s rapidity and azimuth.
%
It also involves a distance measure between every particle $i$ and the
beam
\begin{equation}
...
jet.\footnote{In \fastjet we actually will use PseudoJet to denote any
...
% 
% 
%  (and has been previously, e.g. in \cite{KtClus,KtJet}).
% %
% 
% It is however less straightforward to provide an efficient
% implementation
% 
% 
%  \cite{KtClus,KtJet}.
...
% 
% To understand why this is not an entirely trivial exercise, why a user
% is not necessarily better off simply writing their own code, let us
% first consider the case of a specific sequential recombination
% algorithm, 
% %
% 
%
...
events, this quickly becomes prohibitive, all the more so in
...
Computational Geometry Algorithms Library (CGAL)~\cite{CGAL}, \fastjet
achieves expected $N\ln N$ timing for many sequential recombination
algorithms.
%
...
%  which give identical clustering 
% 
% As is often the case in computational geometry problems, the
% coefficient of $N \ln N$ turns out to be quite large, and to the 
%  and so to
% optimise performance \fastjet uses the geometry in a series of simpler
% ways to achieve fast clustering 
% 
%  for moderate $N \lesssim 30000$.
% 
% 
%  (though for $N \lesssim 30000$ \fastjet usually switches to
% other strategies that are more efficient pre-asymptotically).
...
replaces $p_{ti,j}^2$ with $p_{ti,j}^{-2}$ in
...
%
The main challenge that would be faced by a user wishing to write
their own implementation of a given cone algorithm comes from the
large number of details that enter into a full specification of a cone
algorithm, e.g.\ the precise manner in which stable cones are found,
...
algorithm.
%
Nevertheless, each tends to involve a different interface, a different
4-momentum type, and so forth.
%
%To facilitate the use of a range of different algorithms, 
\fastjet therefore has a ``plugin'' mechanism, which makes it possible
...
%
Jet finding then becomes a multi-scale problem, and in particular
hadronic decays of W's, Z's and top quarks may be sufficiently
...
contamination~\cite{CSSAreas}.
%
Together with a determination of the level of pileup or underlying
event in a specific event, one can then perform event-by-event and
...
substructure tools (indeed, some also serve to remove noise
contamination), and both the substructure and pileup removal make use
...
appendices.
%
Further information is also available from the extensive online
...
% the partonic final states that are relevant in perturbative QCD
% calculations.
% %
...
%   \begin{itemize}
%   \item provide robust, efficient access to the majority of widely
%     used jet algorithms
%   \item framework for tools' interfaces \& some core tools
%   \item idea of extensible.
%   \end{itemize}
% \item Two kinds of algorithm in FJ
%   \begin{itemize}
%   \item Native; many of them involve a certain kind of distance
%     measure ($d_{ij} = ...$); and fast tricks
%   \item External: plugins (though some written by us); restriction on
%     2 to 1
%   \item interface and some helpers for writing your own
%   \end{itemize}
% \item we provide a number of features beyond core jet-finding. 
% \item Areas -- usable (in principle) with any jet finder
% \item Features beyond the jet-finding
%   \begin{itemize}
%   \item background estimators
%   \item tools, including subtractors
%   \end{itemize}
% \item 
% \item subtraction/bkgd-estimation/tools
% \item many other codes before did one or two algs, e.g. KtClus,
%   ArClus, CamJet
% \item computational strategies (factorising geometry and momentum);
%   state our algorithm briefly?
% \item doxygen
% \end{itemize}
...
%%
%% GPS+MC: removed references to CLHEP and KtJet because 
...
reflect the diversity of common usages within the community.
...
The analogous \ttt{+=}, etc., operators, are also defined.\footnote{
  ...
  information of the \texttt{PseudoJet} on the LHS of the operator.  }
...
%
Details are to be found in sections~\ref{sec:genkt} and
\ref{sec:ee-algs}. 
...
object,
%\footnote{The analogue of \ktjet's \ttt{KtEvent}.}  
...
with \ttt{sorted\_by\_pt(...)} function.
...
%  sequence is identical in the two cases.} 
...
\end{lstlisting}
to obtain the list of particles that never took part in the clustering.
%
The following call additionally returns objects that are the result of
...
\begin{lstlisting}
  ...
  // get a (const) pointer to the parent ClusterSequence (throws if inexistent 
  // or no longer valid) 
...
\begin{lstlisting}
  ...
  bool has_area();              // true if the jet has jet-area informaion
...
% With $R > \pi$ and the additional choice of $p=1$, the clustering
% sequence will correspond to that of the $e^+e^-$ $k_t$
% algorithm~\cite{eekt}, often referred to also as the Durham algorithm
...
%   JetDefinition::Plugin * plugin = new SomePluginAlgorithm(...);
%   // use it to initialise a jet definition
%   JetDefinition jet_def(plugin);
% 
%   // then create a ClusterSequence with the input particles and jet_def, and
%   // use it to extract jets as usual; don't forget to delete the plugin afterwards
% \end{lstlisting}
...
\end{lstlisting}
The stable cones are represented as PseudoJets, for which only the
...
Note that the underlying implementation of SISCone is optimized for
large $N$.
%
...
%
The details are indicated for each algorithm as follows (again
following the notation of Ref.~\cite{Salam:2009jx}): IR$_{n+1}$ means
...
%
%It is to be deprecated for new experimental or theoretical analyses.
...
version of SpartyJet \cite{SpartyJet}.
%
...
and Michael Seymour~\cite{PxCone}.  It is also described in
ref.~\cite{Seymour:2006vv}.
...
Jet areas provide a measure of a jet's susceptibility to soft
contamination. 
%
In \fastjet, they are obtained by clustering with the class
\ttt{ClusterSequenceArea} (derived from \ttt{ClusterSequence}). 
%
Its constructor takes an \ttt{AreaDefinition}
argument in addition to the list of particles and the
\ttt{JetDefinition}. A typical usage would look like the following:
\begin{lstlisting}
...
\item The Voronoi area of a jet is the sum of the Voronoi areas of its
  ...
  algorithm (but not ingeneral for other algorithms) the Voronoi area
...
given in \ttt{example/06-area.cc}.

%% When jet areas are to be used to study the level of a diffuse noise
...
When jet areas are to be used to study the level of a diffuse noise
which might be present in the event (like underlying event particles
or pileup) and maybe subtract it from jets, further classes (like
...
distribution. To obtain a reliable area one may then repeat the area
determination several times, the number of times being specified by
the \ttt{repeat} variable.
%
For hadron-level events a value of
$5$ is sufficient to give jet areas that are determined to within a
few percent. 
%
In practice it is usually satisfactory even to set
...
%
If \ttt{repeat} $ > 1$, a statistical uncertainty on the area is
available for each jet, through the \ttt{jet.area\_error()} call, and
...
and \ttt{pt\_scatter}, which  are fractional random fluctuations of the
position of the ghosts on the $y$-$\phi$ grid and of their
transverse momentum; and \ttt{mean\_ghost\_pt} which is the average transverse
...
\ttt{ghost\_maxrap()} (similarly for the other parameters).
%
...
%
This information is often more useful than the geometrical area.
...
%
\footnote{  
\ttt{ClusterSequenceArea} is derived from
\ttt{fastjet::ClusterSequenceAreaBase} (itself derived from
\ttt{ClusterSequence}) and makes use of one among
...
\begin{lstlisting}
  /// Returns the area associated with the given jet
  double area = jet.area();

  /// Returns the error (uncertainty) associated with the determination of the
  /// area of the jet; gives zero when the repeat=1 and for passive/Voronoi areas
...
definition. 
%
We have found that the $k_t$ or Cambridge/Aachen jet algorithm with $R
...
%
Regarding the area definition, an active area with explicit ghosts is
recommended\footnote{With the $k_t$ algorithm one may also use a
  Voronoi area (\ttt{effective\_Rfact = 0.9} is recommended), which
  has the advantage of being deterministic and faster than ghosted
  areas. In this case however one must use a selector which is
  geometrical and selects only jets well within the range of event
  particles. When using ghosts, instead, the selector can go up right
...
% %
% While the grid-based case has been less extensively explored, initial
...
for.
%
Two techniques (described below) can help alleviate this problem, and
the properties of the background are then obtained through the methods
(available for both \ttt{JetMedianBackgroundEstimator} and
...
to centre the selector on the specified jet. Then only the jets that
...
used to estimate $\rho$ or $\sigma$.\footnote{If the selector does not
  ...
  rapidity rescaling is in effect.}
%
...
% , was suggested by unpublished
% studies for the case of $pp$ pileup events 
%(with Pythia~8.145, tune 4C
%for the pileup~\cite{Sjostrand:2007gs}). 
% We have found that an
% alternative method that is adequate for handling the rapidity
% dependence is to 
%
One parametrises the average shape of the rapidity
dependence from some number of pileup events and then carries out an
event-by-event global $\rho$ determination taking into account that
average shape.
%
...
In ongoing studies \cite{GridMedianLH}, we have found that the
rescaling method generally performs comparably to local estimation in
many respects, but with reduced sensitivity to biases in events with
high multiplicities of hard jets.
...
\comment{should remaining part stay here? Go to Appendix since it's a
  bit technical? Maybe easiest to leave it here}
%
...
noise removal, i.e.\ \ttt{Subtrator}. 
%
...
sensitivity of a boosted Higgs-candidate jet to the underlying event. 
%
Given some starting jet obtained with a Cambridge-Aachen clustering
(followed by some Higgs-specific declustering), filtering unclustered
the jet down to a smaller jet radius $R_{\rm filt}$ and only kept the
3 hardest of these subjets as the pieces of the final filtered jet,
...
contamination of soft noise in a jet while aiming to retainin hard
...
% when used together with a mass cut, it also plays the role of
% a boosted-object tagger.
% %
% The noise removal is achieved in pruning in a way which differs from filtering: 
% instead of unclustering a jet and selecting only part of
% its subjets, 
% 
% 
% In practice, recombinations are vetoed in the clustering sequence 
...
standard location. \comment{do we mention issues with 3.7 and 3.8?}
% \footnote{For clustering problems with near
%   degeneracies, there are known issues with CGAL versions 3.7 and
%   3.8. }
...
%% older methods here are not really relevant any more -- and
%% anyway documented in INSTALL
% \ttt{CGAL} may be obtained in source form from
...
The constructor for a \ttt{JetDefinition} takes a strategy argument
...
structures cannot cleanly accomodate multiple particles in the same
...
%
For example for the Cambridge/Aachen jet algorithm with $R=0.4$,
\ttt{NlnNCam} surpasses the \ttt{N2MinHeapTiled} strategy only for
$N\gtrsim 37000$.
%
...
%%%% OLD: and the transitions that have been adopted are 
%%%%      based on a cache size of $2$~MB. 
%%%% 2011-11-17: but seem to work OK also in Gregory's latest
%%%%             6MB cache results       
...
\begin{figure}[t]
  ...
  \caption{Time required to perform the clustering of $N$ particles in
    ...
    %%
    Shown for an Intel I5 760 processor with 6~MB of
    cache~\comment{CHECK}.
    %%
...
of a vector of \ttt{PseudoJet} objects are substantial fraction of the
...
\ttt{PseudoJet}. This is achieved internally with the help of a shared
...
\ttt{area\_4vector()}, \ttt{is\_pure\_ghost()}) will return the information from the
...
They do, however, provide some customisation facilities, especially to
...
\bibitem{SpheriSISCone} G.~P.~Salam and G.~Soyez, 2008,
  unpublished. \comment{Gregory: Can you remember the date}
...
\bibitem{GridMedianLH} M.~Cacciari, G.~P.~Salam and G.~Soyez, contribution in
