...
\title{\sf FastJet user manual%
  \\ \large (for version
  3.4.0-devel%  %%%-VERSION-NUMBER (do not remove this comment; used
...
%
\begin{lstlisting}
  BackgroundEstimate bkgd_estimate = bge();
...
\begin{lstlisting}
  ...
  BackgroundEstimate bkgd_estimate = bge(jet);
...
\end{lstlisting}
Subsequent calls to \ttt{operator()} will be based on the on median of
the distribution $p_t/A / \ttt{rescaling}(y)$ (rather than $p_t/A$).
%
Any calls to \ttt{operator(jet)} will include an additional factor of
...
\ttt{operator(jet)}, but not for calls to \ttt{operator()}
\comment{operator or something else here?} (which are in any case less
meaningful when a rapidity dependence is being assumed for the
background).

In ongoing studies \cite{GridMedianLH}, \comment{Did this ever get
  written up?} we have found that despite its
...
\begin{lstlisting}
  JetMedianBackgroundEstimator::Extras extras = \
                    BackgroundEstimate.extras<JetMedianBackgroundEstimator>();
\end{lstlisting}
...
safe.
%
Concurrent read-access of \ttt{JetDefinition} and
\ttt{AreaDefinition} objects is safe across threads.
%
...
Full thread safety, obtained by configuring with
\ttt{--enable-thread-safety}.
%
Concurrent read-access of \ttt{PseudoJet} objects is now possible,
\ttt{SharedPtr} objects are thread safe, which enables thread safety
across \fastjet operations marked as \ttt{const}.
%
As of version 3.4.0, thread safety is not yet guaranteed in plugins.

Normal \fastjet usage sometimes entails non-const operations and/or
operations that affect global (static) variables.
%
In these case, certain workflows are to be privileged to enable
...
%
With this usage option, a lock is employed to ensure that the full set
of ghosts is generated with a consistent set of random numbers.
%
...
\ttt{ClusterSequenceArea} can use ghosts can be generated using a
specific set of seeds, for example
\begin{lstlisting}
...
\begin{lstlisting}
  ...
  BackgroundEstimate bkgd_estimate1 = (*local_bge)();
  // or locally around some jet
  BackgroundEstimate bkgd_estimate2 = (*local_bge)(jet);
\end{lstlisting}
...
%
In some cases they alter the state of the
\ttt{BackgroundEstimatorBase} object: e.g.\
\ttt{JetMedianBackgroundEstimator::rho(jet)} updates an internal
cache, which then affects the result of calls such as
\ttt{JetMedianBackgroundEstimator::mean\_area()}.
%
With thread-safety enabled at configure-time, the update of the
internal information is carried out in a thread-safe manner if sharing
a background estimator instance for a common event across multiple
threads.
%
This can happen e.g.\ when performing operations on several jets in
parallel, or when applying different groomers in parallel.
%
However, those different threads will be acting on the
cache in ways that are likely to break the link between
the \ttt{rho(jet)} and \ttt{mean\_area()} calls.
%
