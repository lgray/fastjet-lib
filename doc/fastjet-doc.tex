...
\begin{table}[bt]
  \begin{center}
    \begin{tabular}{ll}\toprule
      \ttt{N2Plain} & Plain $N^2$ algorithm (fastest for $N
      \lesssim 30$)\\
      \ttt{N2Tiled} & Tiled $N^2$ algorithm (fastest for $30 \lesssim
      N \lesssim 400$)\\ 
      \ttt{N2MinHeapTiled} & Tiled $N^2$ algorithm with a heap for
      tracking the minimum of \\& $d_{ij}$ 
      %(fastest for $400 \lesssim N \lesssim 15000$)\\ 
      (fastest for $400 \lesssim N \lesssim $ a few thousand for $R < 0.7$)
      \\
      ...
      $N$ and $R$
      \\ 
      \ttt{FJ30Best} & Automatic strategy selection as was done in
      FastJet 3.0
      \\\bottomrule
    \end{tabular}
    \caption{The more interesting of the various algorithmic
      ...
      %
      $N$ ranges in which a given strategy is optimal hold for $R=1$
      and LHC events with full rapidity acceptance. 
      \comment{Change the R to one that's actually widely used?}
      \comment{Leave out FJ10Best?}
    }
...
%
Finally for a given $N$ and $R$, the timings and the optimal strategy
may also depend on the event structure.
