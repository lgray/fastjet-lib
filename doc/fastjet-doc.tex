...
\newcommand{\fjcontrib}{\texttt{fjcontrib}\xspace}
...
subtraction. 
%
The first~\cite{Soyez:2012hv} is related to the fact that hadrons have
masses.
%
...
Eq.~(\ref{eq:BGE-4vector-massless}) must be extended to read
\begin{equation}
...
%
It can be approximately determined as the median across patches of
$m_{\delta,\text{patch}}/A_{\text{patch}}$, where
\begin{equation}
  \label{eq:m-delta}
...
have masses --- or possibly even better, using massless input
particles. \comment{Are we sure???}

\comment{Carry on here: with safe subtraction}.
