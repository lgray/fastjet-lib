...
Tiling25::Tiling25(ClusterSequence & cs) :
...
/// region.
///
void Tiling25::_initialise_tiles() {
...
int Tiling25::_tile_index(const double & eta, const double & phi) const {
  int ieta, iphi;
...
inline void Tiling25::_tj_set_jetinfo( TiledJet * const jet,
...
//----------------------------------------------------------------------
void Tiling25::_bj_remove_from_tiles(TiledJet * const jet) {
  Tile25 * tile = & _tiles[jet->tile_index];
...
void Tiling25::_print_tiles(TiledJet * briefjets ) const {
...
void Tiling25::_add_neighbours_to_tile_union(const int tile_index, 
...
inline void Tiling25::_add_untagged_neighbours_to_tile_union(
...
/// already been tagged.
inline void Tiling25::_add_untagged_neighbours_to_tile_union_using_max_info(
               const TiledJet * jet, 
	       vector<int> & tile_union, int & n_near_tiles)  {
  Tile25 & tile = _tiles[jet->tile_index];
...
inline double Tiling25::_distance_to_tile(const TiledJet * bj, const Tile25 * tile) 
#ifdef INSTRUMENT2
   {
  _ncall_dtt++; // GPS tmp
#else
...
inline void Tiling25::_update_jetX_jetI_NN(TiledJet * jetX, TiledJet * jetI, vector<TiledJet *> & jets_for_minheap) {
...
inline void Tiling25::_set_NN(TiledJet * jetI, 
  ...
  while (n > 0) {
    ...
    if (jetB != NULL) {
      ...
      for (Tile25 ** near_tile  = jetB_tile.begin_tiles; 
        ...
        // second version, which works out distance_to_tile only for jets that
        // have not already been tagged -- at the expense of running over tiles
        // that are not relevant for B.
        // bool relevant = (*near_tile)->tagged;
        // if (!relevant){
        //   double dist_to_tile = _distance_to_tile(jetB, *near_tile);
        //   bool relevant_for_jetB  = dist_to_tile <= jetB->NN_dist;
        //   bool relevant_for_near_tile = dist_to_tile <= (*near_tile)->max_NN_dist;
        //   relevant = relevant_for_jetB || relevant_for_near_tile;
        // }
        // if (relevant) {
        //   for (TiledJet * jetI = (*near_tile)->head; jetI != NULL; jetI = jetI->next) {
        //     if (jetI->NN == jetA || jetI->NN == jetB) _set_NN(jetI, jets_for_minheap);
