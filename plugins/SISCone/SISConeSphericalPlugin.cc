...
// the static objects
auto_ptr<SISConeSphericalPlugin> SISConeSphericalPlugin::stored_plugin    ;
auto_ptr<vector<PseudoJet>     > SISConeSphericalPlugin::stored_particles ;
auto_ptr<CSphsiscone           > SISConeSphericalPlugin::stored_siscone   ;
...
template<> PseudoJet::PseudoJet(const CSphmomentum & four_vector) {
...
void SISConeSphericalPlugin::run_clustering(ClusterSequence & clust_seq) const {
  ...
  if (new_siscone) {
    ...
    vector<CSphmomentum> siscone_momenta(n);
    for(unsigned i = 0; i < n; i++) {
  ...
  SISConeSphericalExtras * extras = new SISConeSphericalExtras;
  for (unsigned ipass = 0; ipass < siscone->protocones_list.size(); ipass++) {
    for (unsigned ipc = 0; ipc < siscone->protocones_list[ipass].size(); ipc++) {
      //siscone->protocones_list[ipass][ipc].build_etaphi();
      //double rap = siscone->protocones_list[ipass][ipc]._eta;
      //double phi = siscone->protocones_list[ipass][ipc]._phi;
      //PseudoJet protocone(cos(phi),sin(phi),sinh(rap),cosh(rap));
      //PseudoJet protocone(cos(phi)*sin(rap),sin(phi)*sin(rap),cos(rap),1.0);
      PseudoJet protocone(siscone->protocones_list[ipass][ipc].px, 
			  siscone->protocones_list[ipass][ipc].py,
			  siscone->protocones_list[ipass][ipc].pz,
			  siscone->protocones_list[ipass][ipc].E);
...
/// 
string SISConeSphericalExtras::description() const {
  ostringstream ostr;
  ostr << "This SISCone (spherical) clustering found " << protocones().size() 
       << " stable protocones";
  return ostr.str();
}
